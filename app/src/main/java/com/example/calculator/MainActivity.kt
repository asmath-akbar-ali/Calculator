...
class MainActivity : AppCompatActivity() {
    ...
    private var inputvalue1: Double? = null
    ...
    private fun onEqualsClicked() {
        if (inputvalue1 != null && inputvalue2 != null && currentOperator != null) {
            result = calculate()
            equation.clear().append(ZERO)
    ...
    private fun updateResultOnDisplay(isPercentage: Boolean = false){
        ...
        val input2Text= getFormattedDisplayValue(value = getInputValue2())
        if (isPercentage) {
            binding.textEquation.text = String.format(
                "%s %s %s",
                getFormattedDisplayValue(value = getInputValue1()),
                getOperatorSymbol(),
                "$input2Text${getString(R.string.percentage)}"
            )
        } else {
            binding.textEquation.text = String.format(
                "%s %s %s",
